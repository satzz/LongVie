...
class FlowMatchScheduler():
    ...
    def step(self, model_output, timestep, sample, to_final=False, **kwargs):
        ...
        # print(sigma_, sigma, timestep)
        prev_sample = sample + model_output * (sigma_ - sigma)
        return prev_sample


    def inverse_step(self, model_output, timestep, sample, to_final=False, **kwargs):
        if isinstance(timestep, torch.Tensor):
            timestep = timestep.cpu()
        timestep_id = torch.argmin((self.timesteps - timestep).abs())
        sigma_list = list(self.sigmas.numpy())+[0]
        timestep_id +=1
        sigma = sigma_list[timestep_id]
        sigma_ = sigma_list[timestep_id - 1]
        # print(sigma_, sigma, timestep)
        prev_sample = sample + model_output * (sigma_ - sigma)
        return prev_sample
