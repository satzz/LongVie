...
class Visualizer:
    ...
    def visualize(
        ...
        if save_video:
            ...
            self.save_video(tracking_video, filename=save_path, writer=writer, step=step)
    ...
    def save_video(self, video, filename, savedir=None, writer=None, step=0):
        if writer is not None:
            ...
            clip = ImageSequenceClip(wide_list, fps=16)
            save_path = filename
            filename = os.path.basename(filename)
            clip.write_videofile(save_path, codec="libx264", fps=16, logger=None)
            print(f"Video saved to {save_path}, {filename}")
